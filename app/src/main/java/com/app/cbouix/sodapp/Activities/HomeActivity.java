    ...
    private void registrarGCM(){
        registrationBroadcastReceiver = new BroadcastReceiver() {
            ...
            public void onReceive(Context context, Intent intent) {
                if(intent.getAction().endsWith(GcmIntentServices.REGISTRATION_SUCCES)){
                    String token = intent.getStringExtra(GcmIntentServices.TOKEN);
                    token.toString();
                    updateGcm(token);
                }else if(intent.getAction().endsWith(GcmIntentServices.REGISTRATION_FAIL)){
    ...
    private void updateGcm(final String token){
        if(!((Activity) this).isFinishing())
        {
            ...
            new Thread(new Runnable() {
                ...
                public void run() {
                    try {
                        UserBusiness.updateGcm(getApplicationContext(), token);
